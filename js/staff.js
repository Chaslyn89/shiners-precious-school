...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadStaffData() {
        try {
            ...
            // ===== BUILD LEADERSHIP CARDS - Using data from staff.json =====
            const leadershipEl = document.getElementById('staff-leadership');
            if (leadershipEl) {
                let leadershipHTML = '';

                // Card 1: Director (from settings)
                leadershipHTML += `
                ...
                // Cards 2+: Other leadership from staff.json
                if (data.leadership && data.leadership.length > 0) {
                    data.leadership.forEach(member => {
                        // Check if member name is not a placeholder
                        if (member.name && member.name !== '[DEPUTY NAME]' && member.name !== '[ACADEMIC COORDINATOR]') {
                            leadershipHTML += `
                                <div class="leader-card">
